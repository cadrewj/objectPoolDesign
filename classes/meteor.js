...
class Meteor {
    ...
    draw(context){
        if(!this.free){ //used to draw only the active meteors
            ...
            context.drawImage(this.image, -this.radius,-this.radius, this.sprtieWidth, this.sprtieHeight) //since we translated the image to position x,y, we only need to offest the image by the radius to get the center 
            context.restore()
        }
    }
    testBound(context){
    ...
    update(context){
        ...
        if(!this.free){ 
            ...
            if(this.x > this.game.width + this.radius){ 
                this.x = -this.sprtieWidth; //reset the position of meteor to offscreen x axis
                this.reset() //remove meteor from the screen by setting its value to free
            }
            else if(this.y > this.game.width  + this.radius){
                this.y = -this.sprtieHeight;  //reset the position of meteor to offscreen on y axis
                this.reset() //remove meteor from the screen by setting its value to free
            }    
        }
    } 
